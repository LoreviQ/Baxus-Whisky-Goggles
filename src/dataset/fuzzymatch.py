...
def find_best_matches(ocr_text: str, data: pd.DataFrame, limit: int = 5) -> list[tuple[int, int]]:
    """
    ...
    if 'name' not in data.columns or 'spirit_type' not in data.columns or 'id' not in data.columns:
        raise ValueError("DataFrame must contain 'id', 'name', and 'spirit_type' columns.")
